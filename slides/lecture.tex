...
\begin{frame}{Two Kinds of Linear Systems}
    \begin{columns}[T]
        ...
        \vspace{0.3cm}
        \begin{tikzpicture}[scale=0.8]
            \node[draw, circle, minimum size=1cm] (A) at (0,0) {$A^{-1}$?};
            \node[left=1cm of A] (x) {$x$?};
            \node[right=1cm of A] (b) {$b$};
            \draw[->, dashed] (A) -- (x);
            \draw[->] (b) -- (A);
...
\begin{frame}{Geometric Intuition}
    ...
    \vspace{0.3cm}
    \includegraphics[width=0.65\textwidth]{figures/steepest_descent_vs_cg.pdf}

    \vspace{0.3cm}
    \begin{itemize}
        \item The contours are \textbf{ellipses} whose shape is determined by $A$
...
\begin{frame}{Least Squares and the Normal Equations}
    ...
    \pause
    \vspace{0.2cm}
    \textbf{We will use this structure for the MRI reconstruction problem.}
\end{frame}
...
\begin{frame}{Why Steepest Descent is Slow}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/steepest_descent_vs_cg.pdf}
    ...
    \pause
    \vspace{0.3cm}
    \begin{block}{Error bound for steepest descent}
        \[
...
\begin{frame}{The Zigzag Problem Visualized}
    \centering
    \includegraphics[width=0.65\textwidth]{figures/sd_vs_cg_error.pdf}
        ...
        \begin{algorithmic}[1]
            ...
            \For{$k = 0, 1, 2, \ldots$ until convergence}
                ...
                \State $\beta_k = \dfrac{r_{k+1}^T r_{k+1}}{r_k^T r_k}$
                      \Comment{conjugacy coefficient}
                \State $p_{k+1} = r_{k+1} + \beta_k p_k$
                      \Comment{new search direction}
            \EndFor
        \end{algorithmic}
    \end{algorithm}
...
\begin{frame}{Convergence Rate: The Condition Number Story}
    \textbf{Error bound in the $A$-norm:}
    \[
        \frac{\|x_k - x^*\|_A}{\|x_0 - x^*\|_A}
    ...
    \begin{itemize}
        \item $\kappa = 1$: Converges in 1 iteration (all eigenvalues equal)
        \item $\kappa = 10$: Factor $\approx 0.52$ per iteration
        \item $\kappa = 100$: Factor $\approx 0.82$ per iteration
        \item $\kappa = 1000$: Factor $\approx 0.94$ per iteration --- much slower
    \end{itemize}

    \pause
    \vspace{0.3cm}
    \begin{block}{Practical consequence}
        Preconditioning: replace $Ax = b$ with $M^{-1}Ax = M^{-1}b$,
...
\begin{frame}{Beyond the Condition Number: Eigenvalue Clustering}
    ...
    \vspace{0.2cm}
    \centering
    \includegraphics[width=0.85\textwidth]{figures/eigenvalue_illustration.pdf}
...
\begin{frame}{Multi-Coil Acquisition: The Key to Acceleration}
    ...
    \vspace{0.3cm}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/mri_acquisition_model.pdf}
...
\begin{frame}{SENSE Normal Equations for CG}
    ...
    \vspace{0.2cm}
    \textbf{Step 3: Solve with CG}\\
    $A_{\text{norm}}$ is Hermitian positive semidefinite --- CG applies directly.
    Each iteration costs $C$ FFT/IFFT pairs.
...
\begin{frame}{Reconstruction Results: Zero-Filled vs CG}
    \centering
    \includegraphics[width=0.9\textwidth]{figures/mri_reconstruction_results.pdf}
...
\begin{frame}{CG Convergence for SENSE Reconstruction}
    \centering
    \includegraphics[width=0.85\textwidth]{figures/mri_cg_convergence.pdf}
...
\begin{frame}{Summary: What We Learned}
    \begin{enumerate}
        ...
        \item \textbf{Steepest descent} is intuitive but suffers from zigzagging
              --- convergence depends on $\kappa(A)$.

        \item \textbf{Conjugate Gradient} fixes this by using $A$-orthogonal
              search directions, converging in at most $n$ steps.
        ...
        \item \textbf{SENSE reconstruction} formulates the problem as a linear
              ...
              matrix-vector products via FFTs.
    \end{enumerate}
